...
Value *TargetLoweringBase::getDefaultCleanStackPointerLocation(IRBuilder<> &IRB,
                                                              bool UseTLS) const {
  // compiler-rt provides a variable with a magic name.  Targets that do not
  // link with compiler-rt may also provide such a variable.
  Module *M = IRB.GetInsertBlock()->getParent()->getParent();
  const char *UncleanStackPtrVar = "__cleanstack_unclean_stack_ptr";
  auto UncleanStackPtr =
      dyn_cast_or_null<GlobalVariable>(M->getNamedValue(UncleanStackPtrVar));
  ...
  if (!UncleanStackPtr) {
    ...
    // We use the initial-exec TLS model because we do not support the
    // variable living anywhere other than in the main executable.
    UncleanStackPtr = new GlobalVariable(
        *M, StackPtrTy, false, GlobalValue::ExternalLinkage, nullptr,
        UncleanStackPtrVar, nullptr, TLSModel);
  } else {
    // The variable exists, check its type and attributes.
    if (UncleanStackPtr->getValueType() != StackPtrTy)
      report_fatal_error(Twine(UncleanStackPtrVar) + " must have void* type");
    if (UseTLS != UncleanStackPtr->isThreadLocal())
      report_fatal_error(Twine(UncleanStackPtrVar) + " must " +
                         (UseTLS ? "" : "not ") + "be thread-local");
  }
  return UncleanStackPtr;
}

Value *TargetLoweringBase::getCleanStackPointerLocation(IRBuilder<> &IRB) const {
  if (!TM.getTargetTriple().isAndroid())
    return getDefaultCleanStackPointerLocation(IRB, true);

  // Android provides a libc function to retrieve the address of the current
  // thread's unclean stack pointer.
  Module *M = IRB.GetInsertBlock()->getParent()->getParent();
  Type *StackPtrTy = Type::getInt8PtrTy(M->getContext());
  FunctionCallee Fn = M->getOrInsertFunction("__cleanstack_pointer_address",
                                             StackPtrTy->getPointerTo(0));
  return IRB.CreateCall(Fn);
}
