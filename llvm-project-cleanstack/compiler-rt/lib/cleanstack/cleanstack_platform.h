...
//===----------------------------------------------------------------------===//
//
// This file implements platform specific parts of CleanStack runtime.
//
//===----------------------------------------------------------------------===//

#ifndef CLEANSTACK_PLATFORM_H
#define CLEANSTACK_PLATFORM_H
...
static void *GetRealLibcAddress(const char *symbol) {
  ...
  if (!real) {
    fprintf(stderr, "cleanstack GetRealLibcAddress failed for symbol=%s",
            symbol);
    abort();
  }
  return real;
