// RUN: %clang_cleanstack -fno-stack-protector -D_FORTIFY_SOURCE=0 -g %s -o %t.nossp
// RUN: %run %t.nossp 2>&1 | FileCheck --check-prefix=NOSSP %s

// RUN: %clang_cleanstack -fstack-protector-all -D_FORTIFY_SOURCE=0 -g %s -o %t.ssp
// RUN: env LIBC_FATAL_STDERR_=1 not --crash %run %t.ssp 2>&1 | \
// RUN:     FileCheck -check-prefix=SSP %s

// Test stack canaries on the unclean stack.
...
__attribute__((noinline)) void f(unsigned *y) {
  ...
  // the unclean stack and this should not touch anything important.
